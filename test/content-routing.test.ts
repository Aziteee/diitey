...
describe("complete content and routing model", () => {
  ...
  test("collection results use declared ordering and content ID as a stable tie-breaker", async () => {
    const siteRoot = await copyFixtureSite();
    await rm(join(siteRoot, "content", "hello.md"));
    await writeContent(siteRoot, "articles/z.md", {
      id: "bravo",
      created: "2026-07-12",
      title: "Bravo",
    });
    await writeContent(siteRoot, "articles/a.md", {
      id: "alpha",
      created: "2026-07-12",
      title: "Alpha",
    });
    await writeContent(siteRoot, "articles/new.md", {
      id: "newest",
      created: "2026-07-13",
      title: "Newest",
    });
    await writePage(
      ...
      export default function List({ items }: { items: readonly ContentRecord[] }) {
        return <ol>{items.map((item) => <li>{item.id}</li>)}</ol>;
      }`,
    );
    await writeTheme(
      siteRoot,
    ...
    expect(html).toContain("<ol><li>newest</li><li>alpha</li><li>bravo</li></ol>");
  }, 10_000);

  test("collection results apply declared filters without implicit conversion", async () => {
    const siteRoot = await copyFixtureSite();
    await rm(join(siteRoot, "content", "hello.md"));
    await writeContent(siteRoot, "articles/published.md", {
      id: "published",
      created: "2026-07-12",
      title: "Published",
      draft: false,
    ...
    await writeContent(siteRoot, "articles/draft.md", {
      id: "draft",
      created: "2026-07-13",
      title: "Draft",
      draft: true,
    ...
    await writeContent(siteRoot, "articles/plain.md", {
      id: "plain",
      created: "2026-07-14",
      title: "Plain",
      draft: false,
    ...
    await writeTheme(
      ...
      `export default defineTheme({
        collections: {
          writing: collection({
            from: "articles/*.md",
            where: { draft: { not: true }, tags: { contains: "featured" } },
            schema: { title: "string", draft: "boolean?", tags: "string[]?" },
          }),
        },
    ...
    expect(html).toContain("<p>published</p>");
  }, 10_000);
  ...
  test("site owner cannot sort a collection containing incompatible field types", async () => {
    const siteRoot = await copyFixtureSite();
    await rm(join(siteRoot, "content", "hello.md"));
    await writeContent(siteRoot, "articles/numeric.md", {
      id: "numeric",
      created: "2026-07-12",
      title: "Numeric",
      rating: 5,
    });
    await writeContent(siteRoot, "articles/text.md", {
      id: "text",
      created: "2026-07-13",
      title: "Text",
      rating: "5",
    });
    await writePage(siteRoot, "list", `export default function List() { return <p>list</p>; }`);
    await writeTheme(
      siteRoot,
      `export default defineTheme({
        collections: {
    ...
    expect(error).toContain("rating has incompatible types number and string");
  }, 10_000);

  test("site owner cannot load equivalent parameterized route patterns", async () => {
    const siteRoot = await copyFixtureSite();
    await rm(join(siteRoot, "content", "hello.md"));
    await writeContent(siteRoot, "articles/hello.md", {
      id: "article",
    ...
    await writeContent(siteRoot, "notes/world.md", {
      id: "note",
      created: "2026-07-13",
      title: "Note",
    });
    await writeTheme(
      ...
      `export default defineTheme({
        collections: {
          articles: collection({ from: "articles/*.md", schema: { title: "string" } }),
          notes: collection({ from: "notes/*.md", schema: { title: "string" } }),
        },
        routes: [
          route("/writing/:slug", page("article", {
            item: { collection: "articles", match: "articles/:slug.md" },
          })),
          route("/writing/:name", page("article", {
            item: { collection: "notes", match: "notes/:name.md" },
          })),
        ],
      });`,
    );
