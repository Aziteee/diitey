...
const todoListConfig = z
  .object({
    maxTitleLength: z.number().int().positive().max(1_000),
  })
  .strict()
  .default({ maxTitleLength: 100 });
...
export default definePlugin({
  ...
  setup(config) {
    const createInput = z
      .object({
        title: z.string().trim().min(1).max(config.maxTitleLength),
      })
      .strict();
          ...
          handler(input, { database }) {
            ...
            if (result.changes === 0) {
              throw new PluginNotFoundError(
                `Todo item ${input.id} does not exist`,
              );
            }
            const row = database
              ...
              .get(input.id);
            if (!row) {
              throw new PluginNotFoundError("Todo item does not exist");
            }
